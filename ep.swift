...
{
   preprocess @execute @input stdout=@out;
}

app (file out, file csv, file sout,file serr) runEP (file execute, file input, string casename)
{
   run @execute @input casename @out @csv stdout=@sout stderr=@serr;
}

app (file out, file csv, file sout,file serr) postProcess (file execute, file[] cases,file[] casescsv, file[] casesout, file[] caseserr)
{
   postprocess @execute @out @csv stdout=@sout stderr=@serr;
}
...
# ----- PARAMETER SETUP & LAUNCH
  ...
  foreach caseName, i in caseNames {
    	...
    	(runcase, casecsv, caseout,caseerr) = runEP(RUNEP,input,caseName);
    	cases[i] = runcase;
    	casescsv[i] = casecsv;
    	casesout[i] = caseout;
    	caseserr[i] = caseerr;
...
trace(filenames(cases));
  ...
  (finalresults, totalcsv, pout,perr) = postProcess(POSTPROCESS,cases,casescsv,casesout,caseserr);
